...
// Data
import { portfolioClassicData } from "../../Global/Portfolio/PortfolioData";
  ...
  function showImgContent(e) {
    ...
    document.querySelectorAll(".portfolio-caption-wrapper").forEach((item) => {
      ...
      if (x + el.clientWidth > item.clientWidth + item.offsetLeft) {
        el.style.transform = `translate3d(${x - el.clientWidth}px, ${
          y + 10
        }px, 0)`;
      }
      return undefined;
    ...
    <div className="grid-wrapper">
      ...
      <ul
        ...
        className={`grid-container text-center ${
          props.filter === false ? "mt-0" : "mt-24 md:mt-[4.5rem] sm:mt-8"
      ...
      >
        ...
        {props.data.map((item, i) => {
          return (
            <li
              key={i}
              ref={portfolioCaption.current[i]}
              className={`grid-item portfolio-caption-wrapper${
                item.double_col ? " grid-item-double" : ""
              } ${item.category.toString().split(",").join(" ").toLowerCase()}`}
            >
              <m.div
                className="portfolio-justified cursor-pointer"
                ...
                <div className="portfolio-image">
                  ...
                  {(item.title || item.subtitle) && (
                    <div className="portfolio-caption m-0 bg-[#fff] text-inherit pt-[15px] px-[25px] pb-[12px] whitespace-nowrap z-[1] shadow-xl text-start">
                      {item.title && (
                        <span className="block text-md font-light">
                          {item.title}
                        </span>
                      )}
                      {item.subtitle && (
                        <span className="block font-medium text-darkgray text-md font-serif uppercase">
                          {item.subtitle}
                        </span>
                      )}
                    </div>
      ...
      {isOpen && (
        <Lightbox
          imageCaption={`${props.data[photoIndex].subtitle}... ${
            photoIndex + 1
