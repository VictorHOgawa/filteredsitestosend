  ...
  useEffect(() => {
    const fetchData = async () => {
      const url = await fetch(
        `https://graph.instagram.com/me/media?fields=id,media_type,media_url,timestamp,permalink,comments_count,like_count&access_token=${process.env.REACT_APP_INSTA_API_KEY}`
      );
  ...
  return props.carousel !== true ? (
    <div className="grid-wrapper">
      ...
      >
        ...
        {instaFeed &&
          instaFeed.slice(0, props.total_posts).map((item, i) => {
            return (
